...
describe('listClassesForAdmin', () => {
  it('is_super면 전체 수업 + 미배정 수업 가상 항목을 반환한다', async () => {
    const mockOrder = vi.fn().mockResolvedValue({ data: [{ id: 'class-1', name: '3학년 2반' }], error: null })
  ...
  it('일반 관리자는 admin_class_access로 연결된 수업만 반환한다', async () => {
    const mockEq = vi.fn().mockResolvedValue({
      data: [{ classes: { id: 'class-1', name: '3학년 2반' } }],
      error: null,
    })
    ...
    expect(classes).toEqual([{ id: 'class-1', name: '3학년 2반' }])
  })
})
